...
public class PA2 extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
	...
	public PA2() {
		...
		this.dragonfly = new DragonFly(INITIAL_POSITION, "dragonfly");
	}
	...
	public void display(final GLAutoDrawable drawable) {
		...
		if (this.stateChanged) {
			this.dragonfly.update(gl);
			this.stateChanged = false;
		}

		// redraw the components
		this.dragonfly.draw(gl);
	}
	...
	public void keyPressed(final KeyEvent key) {
		...
		case KeyEvent.VK_UP:
			...
			this.dragonfly.changeSelected(bc);
	...
	private void setModelState(final Map<String, Configuration> state) {
		...
		this.dragonfly.setModelStates(state);
	 ...
	 * 
	 * 7 : toggle the foredragonfly for rotation
	 * 
	 * 8 : toggle the upper dragonfly for rotation
	 * 
	...
	private void printJoints(final PrintStream printStream) {
		this.dragonfly.printJoints(printStream);
	}
	...
	public void mouseDragged(final MouseEvent mouse) {
		if (this.rotate_world) {
			 ...
			 */
			//create the unit vector in the direciton of (dy, dx, 0)
			double magnitude = Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2));
			float[] axis;
			if (magnitude == 0) {
				axis = new float[] {1, 0, 0}; 
			}else {
				axis = new float[] {(float) (dy/magnitude), (float) (dx/magnitude), 0};
			}
			
			//calculate quaternion
			float viewDelta = (float) Math.PI/ 180.0f;
			float sin = (float) Math.sin(0.5f * viewDelta);
			float cos = (float) Math.cos(0.5f * viewDelta);
	...
	private void toggleSelection(final int componentNum) {
		this.dragonfly.toggleSelection(componentNum);
